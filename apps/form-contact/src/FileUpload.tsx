...
export interface FileUploadProps {
  ...
  disabled?: boolean;
  temporaryUploads?: boolean; // Stöd för uppladdningar utan submissionId
}
...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const uploadFiles = async () => {
    ...
    if (!temporaryUploads && !submissionId) {
      alert('Submission ID saknas');
      return;
    }
    ...
    try {
      ...
      let url;
      if (temporaryUploads) {
        url = `${apiUrl}/files/upload/temp`;
      } else {
        url = `${apiUrl}/files/upload/${submissionId}`;
      }
      ...
      {selectedFiles.length > 0 && (
        <div className="selected-files">
          ...
          <button
            ...
            onClick={uploadFiles}
            disabled={uploading || (!temporaryUploads && !submissionId) || selectedFiles.length === 0}
          >
